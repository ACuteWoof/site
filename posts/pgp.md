...
description: Arguing that you don't care about the right to privacy
  because you have nothing to hide is no different from saying you don't
  care about free speech because you have nothing to say. -- Edward
  Snowden
lang: en
theme-color: "#689d6a"
...
[PGP](https://en.wikipedia.org/wiki/Pretty_Good_Privacy) encrypts a
message by first creating a session key, encrypting the message with the
session key, and finally encrypting the session key with the receiver's
public key.
...
shouldn't be), you can use GPG4Win on Windows or `gnupg` through
Homebrew.

If you want a GUI, take a look at
<https://www.gnupg.org/software/frontends.html>. We will be using the
CLI.
...
`gpg --import /path/to/key`. For example, say I want to import the
example key I generated:
...
each message as a file. For an ASCII output, you simply add `--armor`
and change the output file path to something more sensible like `.txt`
(the file extension really is irrelevant to GPG).
...
When encrypting, you would also want to add `--sign` to sign the
message, proving that the message is from your key pair.
...
That should cover enough to get you started. You should read the [docs
for GPG](https://www.gnupg.org/documentation/).
